...
```
IQ Gateway
    │  HTTPS + JWT (configurable interval)
    ▼
Collector ──► Window Aggregator ──► SQLite
                                       │
                                       ▼
                               Axum REST API ◄── Bearer Auth (optional)
                                       │
                                       ▼
                                  HTTP clients
```
...
| Option | When to use |
|--------|-------------|
| **Binary from source** | Development, or when Docker is unavailable |
| **Docker run** | Quick single-container evaluation |
| **Docker Compose** | Production on a home server; includes restart policy and named volume |
| **Behind Caddy** | Any time the API is exposed beyond `localhost`; Caddy auto-provisions HTTPS |

See the [README](./README.md#installation) for step-by-step instructions for each option.
